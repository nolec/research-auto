...
Task 3는 deterministic calibration baseline, evaluator, provenance-bound metric bundle generator까지 구현됐다. 네 source의 실제 artifact 경로는 versioned manifest로 고정됐으며, 원본은 Git에서 제외된 `local_ignored` custody로 유지한다. 기본 suite는 fixture만 사용하고 실제 40건 projection은 명시적인 opt-in integration으로 검증한다. 현재 코드는 gold-free inference corpus와 human-label sidecar를 분리하고, qualification·packet·corpus·gold·구현 파일 hash를 preflight에서 결속한 뒤 `rule_v1`의 P/M/usable-evidence confusion matrix, precision/recall, coverage, invalid/abstention, source별 breakdown을 exact-schema aggregate bundle로 원자적으로 저장할 수 있다. Raw output은 저장하지 않으며 기존 bundle은 조용히 덮어쓰지 않는다. 40건 baseline dry-run은 valid 40, invalid 0, abstention 22, money positive 7이었고 전체 **518 tests**가 통과하며 local integration 1건은 기본 실행에서 의도적으로 skip된다. 아직 승인된 코드 checkpoint에서 실제 baseline metric bundle을 생성하지 않았고, gate freeze receipt, 모델 기반 추출, clustering, Opportunity Card, TOP 20도 구현되지 않았다.
