...
Task 3는 deterministic calibration baseline까지 구현됐다. 현재 코드는 네 source에서 development 10건씩 총 40건을 projection하고, gold-free inference corpus와 human-label sidecar를 분리하며, `rule_v1`이 contract-valid extraction과 abstention receipt를 재현 가능하게 생성한다. 40건 dry-run은 valid 40, invalid 0, abstention 22, money positive 7이었고 전체 **498 tests**가 통과한다. 다만 이는 비교 기준용 규칙 baseline이며 실제 모델 기반 문제 추출, clustering, Opportunity Card, TOP 20은 아직 구현되지 않았다. 따라서 현재 제품은 데이터 수집과 extractor calibration에는 사용할 수 있지만 사업 기회 탐지 제품으로 실제 사용할 수 있는 상태는 아니다.
