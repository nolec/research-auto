...
_SCHEMA = json.loads((Path(__file__).resolve().parents[2] / "schemas/stackexchange-smoke-manifest.schema.json").read_text())
_VALIDATOR = Draft202012Validator(_SCHEMA, format_checker=FORMAT_CHECKER)


def validate_stackexchange_smoke_manifest(
    manifest: Mapping[str, object], compliance: Mapping[str, object], filter_record: Mapping[str, object]
) -> list[str]:
    errors = [
        f"{'.'.join(str(part) for part in error.absolute_path) or '$'}: {error.message}"
        for error in sorted(_VALIDATOR.iter_errors(manifest), key=lambda value: tuple(map(str, value.absolute_path)))
    ]
    if errors:
        return errors
    ...
    if any(count > 1 for count in Counter(names).values()):
        errors.append("sites must be unique")
    if sum(int(site["quota"]) for site in sites) != int(manifest["target_valid_records"]):
        errors.append("site quotas must total target_valid_records")
    ...
    if validate_frozen_filter(filter_record):
        errors.append("frozen filter is invalid")
    elif content_sha256(filter_record) != manifest["filter_hash"]:
        errors.append("filter hash mismatch")
    if content_sha256(compliance) != manifest["compliance_hash"]:
        errors.append("compliance hash mismatch")
    elif compliance.get("source") != "stackexchange" or compliance.get("decision") != manifest["compliance_decision"]:
